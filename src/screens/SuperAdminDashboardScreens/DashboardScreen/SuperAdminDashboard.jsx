...
const SuperAdminDashboard = () => {
  ...
  useEffect(() => {
    // Fetch dashboard statistics, landlords, and all residents from API
      ...
      try {
        ...
        if (statsResponse.ok && statsResult.success) {
          setStats({
            ...
            total_admins: statsResult.data?.stats?.total_admins || 0,
            total_residents: 0, // Will be updated from residents fetch
          });
        }
        ...
        if (landlordsResponse.ok && landlordsResult.success) {
          setLandlords(landlordsResult.data || []);
        }

        // Calculate total residents from all landlords
        if (landlordsResult.success && landlordsResult.data) {
          const totalResidentsCount = landlordsResult.data.reduce(
            (total, landlord) => {
              return total + (landlord.residents_count || 0);
            },
            0
          );
          ...
          setStats((prev) => ({
            ...prev,
            total_residents: totalResidentsCount,
          }));
        }
      } catch (error) {
      ...
      <div className="w-full px-0">
        <div className="max-w-full mx-auto sm:px-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 mt-8">
            ...
            <div
              className={`${theme.background.card} rounded-xl ${theme.shadow.small} p-4 sm:p-6 text-center col-span-2 sm:col-span-1`}
            >
              <Icon
