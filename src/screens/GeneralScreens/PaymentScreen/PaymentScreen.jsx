...
const PaymentScreen = () => {
  ...
  const initializeFlutterwavePayment = async (plan, packageType) => {
    try {
      ...
      const backendPackageType =
        packageType === "user_package" ? "tenant" : packageType;
      ...
      if (!window.FlutterwaveCheckout) {
        ...
        const checkScript = setInterval(() => {
          ...
          if (window.FlutterwaveCheckout) {
            ...
            logPayment(
              "✅ Flutterwave script loaded after " + retries + " attempts"
            );
            initiateFlutterwavePayment(paymentData, plan);
          } else if (retries > 20) {
            clearInterval(checkScript);
            logPayment(
              "❌ Flutterwave script failed to load after 20 attempts"
            );
            alert(
    ...
    try {
      window.FlutterwaveCheckout({
        ...
        callback: async function (response) {
          ...
          alert(
            "⏳ Payment callback received! Processing verification...\n\nDO NOT CLOSE THIS PAGE!"
          );
          ...
          if (response.tx_ref) {
            ...
            try {
              ...
              logPayment(
                "Verification URL",
                `${API_BASE_URL}/api/payments/verify/${response.tx_ref}`
              );
              logPayment(
                "Auth Token Status",
              ...
              try {
                ...
                logPayment(
                  "📥 Backend verification response parsed",
                  verifyResult
                );
              } catch {
                alert(
                  `⚠️ Payment successful but response parsing failed.\nPlease contact support with reference: ${response.tx_ref}`
                );
                setIsPaymentLoading(false);
                return;
              }
              ...
              if (verifyResponse.ok && verifyResult && verifyResult.success) {
                ...
                alert(
                  `🎉 Payment Successful & Verified!\nTransaction ID: ${
                    ...
                    verifyResult.data.user_payment_count
        ...
        onclose: function () {
          logPayment(
            "🔒 Flutterwave payment modal closed (User cancelled or closed)"
          );
                ...
                <div className="text-center mb-6">
                  ...
                  <div className="mt-4 flex justify-end px-4">
                    <button
                      ...
                      className="px-4 py-2 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-medium transition-all shadow-md hover:shadow-lg flex items-center gap-2"
                    >
